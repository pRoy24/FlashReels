...
async function verifySavedRuntimeKeys(
  expected: Partial<Record<SecretName, string>>,
  encryptionKey: Buffer,
) {
  const store = await readStore();
  const failedKeys = Object.entries(expected)
    .filter(([, value]) => value)
    .filter(([name, value]) => decrypt(store.keys[name as SecretName], encryptionKey) !== value)
    .map(([name]) => name);

  if (failedKeys.length) {
    throw apiError(`Saved setup could not be read back from persistent storage: ${failedKeys.join(", ")}. Check the configured Redis store and redeploy with the correct environment variables.`, 500);
  }
}
...
export async function saveRuntimeKeys(payload: Record<string, unknown>, request?: Request): Promise<SaveRuntimeKeysResult> {
  ...
  await verifySavedRuntimeKeys({ samsarApiKey, runwayApiKey, serverSecret }, encryptionKey);
