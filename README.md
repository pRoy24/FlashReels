...
On Vercel, runtime code cannot persist changes to deployment env files. Connect Vercel Redis or Upstash Redis so `REDIS_URL`, `KV_URL`, `KV_REST_API_URL` / `KV_REST_API_TOKEN`, `REDIS_REST_API_URL` / `REDIS_REST_API_TOKEN`, or `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` are injected; the authenticated wizard will then save encrypted secrets to Redis and bootstrap a persistent encryption seed automatically. If you do not want runtime secret storage, set `FLASHREELS_SAMSAR_API_KEY` directly in the Vercel project environment variables instead.
